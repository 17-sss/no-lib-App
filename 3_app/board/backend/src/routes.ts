...
router.get("/post/:id?", async (req: Request, res: Response) => {
  try {
    ...
    if (isError || !postData) throw new ServerError(createMessage({ type: "NOT_FOUND_POST_DATA" }), BAD_REQUEST);
...
router.post("/write", async (req: Request, res: Response) => {
  try {
    if (!req.body) throw new ServerError(createMessage({ type: "NO_DATA_SENT" }), BAD_REQUEST);
    ...
    if (id === -1) throw new ServerError(createMessage({ type: "UNABLE_WRITE" }), BAD_REQUEST);
    ...
    const result: ResponseDataType<number> = {
      data: id,
      message: createMessage({ type: "SUCCESS" }),
      statusCode: OK,
    };
    return res.status(OK).json(result);
  } catch (e) {
...
router.put("/edit", async (req: Request, res: Response) => {
  try {
    ...
    if (!allData || !Array.isArray(allData))
      throw new ServerError(createMessage({ type: "UNABLE_EDIT" }), INTERNAL_SERVER_ERROR);

    const body: PostData = req.body;
    const findIdx = allData.findIndex((v) => body.id && v.id === +body.id);
    if (findIdx === -1) throw new ServerError(createMessage({ type: "UNABLE_EDIT" }), INTERNAL_SERVER_ERROR);
...
router.delete("/delete", async (req: Request, res: Response) => {
  try {
    ...
    const { result: allData } = getPostsData({ jsonFileName });
    if (!allData || !Array.isArray(allData))
      throw new ServerError(createMessage({ type: "UNABLE_DELETE" }), INTERNAL_SERVER_ERROR);

    const id: number = +req.body.id;
    if (Number.isNaN(id)) throw new ServerError(createMessage({ type: "UNABLE_DELETE" }), INTERNAL_SERVER_ERROR);
    ...
    const result = { data: null, message: createMessage({ type: "SUCCESS" }), statusCode: OK };
    return res.status(OK).json(result);
  } catch (e) {
    console.error(e);
    const message = (e as Error).message;
